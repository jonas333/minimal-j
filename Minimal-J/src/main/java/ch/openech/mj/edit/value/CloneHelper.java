...
public class CloneHelper {
	...
	private static void _deepClone(Object from, Object to)
		...
		for (Field field : from.getClass().getDeclaredFields()) {
			...
			} else if (isPrimitive(field) || FieldUtils.isTransient(field)
					|| fromValue == null) {
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private static void _deepCopy(Object from, Object to)
			throws IllegalArgumentException, IllegalAccessException {
		for (Field field : from.getClass().getDeclaredFields()) {
			if (FieldUtils.isStatic(field))
				continue;
	...
	public static boolean isPrimitive(Field field) {
		if (field.getType().getName().startsWith("java"))
			return true;
		if (field.getType().getName().startsWith("org.joda"))
			return true;
