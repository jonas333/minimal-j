...
public class MainActivity extends Activity {
	...
	private void initActivity() {
		...
		IComponent[] components = { new AndroidLabel(this, "Erster Eintrag"),
				new AndroidLabel(this, "Zweiter Eintrag") };

		gridLayout.addView((View) ClientToolkit.getToolkit()
				.createHorizontalLayout(components));
		...
		for (int i = 0; i < 2; i++) {
			for (int j = 0; j < 4; j++) {
				gl.add(ClientToolkit.getToolkit().createTitle(
						AndroidHelper.createRandomString("GRID", 25)), 1);
			}
		}
		gl.add(ClientToolkit.getToolkit().createTitle(
				AndroidHelper.createRandomString("GRID", 50)), 2);
		...
		SwitchLayout switchLayout = ClientToolkit.getToolkit()
				.createSwitchLayout();
		switchLayout.show(ClientToolkit.getToolkit().createLabel(
				"A SwitchLayout Label"));
	...
	private void createTable(GridLayout content) {
		TextView label = new TextView(this);
		label.setText("Table");
		content.addView(label);
		Lookup<Book> lookup = new Lookup<Book>() {
		...
		Integer[] ids = { 1, 2 };
		ITable<Book> table = ClientToolkit.getToolkit().createTable(lookup,
				BookTablePage.FIELDS);
		table.setIds(Arrays.asList(ids));
		content.addView((View) table);
	}
	...
	private void createCheckBox(ViewGroup content) {
		...
		content.addView((View) ClientToolkit.getToolkit().createCheckBox(l,
				"Check me"));
	}
	...
	private void createDialog(ViewGroup content) {
		...
		bDialog.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (!visible) {
					dlg = ClientToolkit.getToolkit().createDialog(
							null,
							"Ein Beispieldialog",
							ClientToolkit.getToolkit().createTextField(null,
									10, null), (IAction[]) null);
					dlg.openDialog();
				} else {
					dlg.closeDialog();
				}
	...
	private void createConfirmationDialog(ViewGroup content) {
		...
		confirmationDialog.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				ClientToolkit.getToolkit().showConfirmDialog(null,
						"Bestätigen",
	...
	private void createMessageDialog(ViewGroup content) {
		...
		messageDialog.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				ClientToolkit.getToolkit().showMessage(this,
	...
	private void createErrorDialog(ViewGroup content) {
		...
		errorDialog.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				ClientToolkit.getToolkit().showError(this,
	...
	private void createLink(ViewGroup content) {
		...
		label.setText("Link");
		content.addView(label);
		content.addView((View) ClientToolkit.getToolkit().createLink("",
				"http://www.google.de"));
	}
	...
	private void createTitle(ViewGroup content) {
		...
		content.addView(label);
		content.addView((View) ClientToolkit.getToolkit().createTitle(
	...
	private void createComponentAndCaption(ViewGroup content) {
		...
		content.addView((View) new AndroidSampleCaption(this, "Eingabe",
				(View) et));
	}
	...
	private void createReadOnlyTextfield(ViewGroup content) {
		...
		content.addView(label);
		content.addView((View) ClientToolkit.getToolkit()
	...
	private void createActionLabel(GridLayout content) {
		...
		content.addView(label);
		content.addView((View) ClientToolkit.getToolkit().createLabel(
